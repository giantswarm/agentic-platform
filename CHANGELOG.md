...
- Shared muster `RemoteMCPServer` for tenant agents (`agents.muster.shared`, umbrella + connectivity `values.yaml`). The connectivity chart now renders a single `RemoteMCPServer` named `muster` in the release namespace (`agentic-platform`) with `allowedNamespaces: {from: All}`, so agents in any namespace can reference the muster gateway cross-namespace — this is the admin-owned server the generic [`agent` chart](https://github.com/giantswarm/agent) defaults its `serverRef` to (`muster`/`agentic-platform`), per the [creating-agents PRD](https://github.com/giantswarm/bumblebee-plans/blob/main/creating-agents/PRD.md). On-behalf-of only: unlike the per-agent `muster-<name>` servers (which inject a static per-agent SA token via `headersFrom`), the shared server sets no `headersFrom`, so the caller token forwarded by kagent (`KAGENT_PROPAGATE_TOKEN`) is the only `Authorization` reaching muster, and muster authenticates every call. Enabled by default whenever `kagent.enabled` and `muster.enabled`; disable via `agents.muster.shared.enabled`. The name is fixed by the template — it is the platform contract the agent chart's default `serverRef` depends on, so it is deliberately not configurable.
...
- `agents.definitions.<agent>`: agents in on-behalf-of mode (`obo.enabled: true`) now drop the static SA-token injection on their muster `RemoteMCPServer` — the `headersFrom` `Authorization` (and the previous `X-Actor-Token` act-chain header) are no longer rendered, so kagent's propagated human muster access_token (forwarded via `KAGENT_PROPAGATE_TOKEN`) is the only `Authorization` reaching muster and the call runs as the real user instead of being shadowed by the agent SA (kagent applies static headers last, so they would otherwise win). The forwarded token is already a muster token, so no RFC 8693 act-chain is used. Because a single request carries one `Authorization`, the modes are mutually exclusive: a tokenless autonomous (M2M) run cannot reach muster while `obo.enabled`, so keep it disabled for agents that must run autonomously against muster (this awaits kagent-dev/kagent#2087, which would keep the static SA token as an M2M fallback while a propagated user token still wins). Agents with `obo.enabled: false` keep the unchanged pure-M2M static-SA path. The per-agent token-mint Job/CronJob/Secret remain rendered but are unused by OBO agents. This is the chart side of making the Slack-OBO sre-agent act as the human against muster; the downstream mcp-kubernetes/mcp-prometheus local-mint leg is unchanged.
