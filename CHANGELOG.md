...
- `kagent.controllerRoute.jwtAuthentication` (umbrella + connectivity `values.yaml` and `values.schema.json`): opt-in JWT validation on the inner `kagent-controller` HTTPRoute. When enabled, the connectivity chart renders an `AgentgatewayPolicy` (dex issuer, no required audiences) and a static `AgentgatewayBackend` (`dex-jwks`) for the JWKS endpoint, so agentgateway verifies the bearer's signature, issuer, and expiry against dex before forwarding to the kagent controller. `kagent.controllerRoute.jwtAuthentication.mode` selects the validation strictness (enum `Strict`, `Optional`, `Permissive`; default `Strict`): `Strict` rejects requests with a missing or invalid token, `Optional` validates a token when present but allows requests with none, `Permissive` validates without rejecting so an install can confirm real traffic carries valid dex tokens before enforcing. This makes the trusted-proxy assumption of `kagent.controller.auth.mode: trusted-proxy` actually hold on the A2A path; previously no policy existed and any parseable JWT passed. Off by default: enabling it rejects callers that do not present a dex-issued JWT (opaque muster tokens, Kubernetes ServiceAccount tokens), so enable it per install only once all A2A callers forward dex JWTs. Requires `gateway.jwksEgress.enabled: true` so the data plane can reach dex. A `ci/test-kagent-jwt-policy-values.yaml` render test covers the new resources, and a render-time guard fails when it is enabled without an issuer.
