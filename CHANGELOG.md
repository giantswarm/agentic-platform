...
- `Chart.yaml` description updated to reflect the runtime contract (muster + agentgateway + opt-in Valkey; CRDs as cluster prerequisite) and `appVersion` tracks the bundled `muster` sub-chart (`0.1.193`) so operator dashboards querying `app.kubernetes.io/version` get a meaningful value.
