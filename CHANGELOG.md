...
- `agents.definitions.<agent>`: agents that forward the human token (on-behalf-of) now drop their static SA-token injection. When an agent's `allowedHeaders` includes `authorization`, its muster `RemoteMCPServer` omits the `headersFrom` SA Bearer injection (and the per-agent token-mint Job, refresh CronJob, and Secret-writer RBAC are no longer rendered) so the propagated human muster token reaches muster as the real user instead of being shadowed by the agent SA. A single request carries one `Authorization` header, so the two are mutually exclusive: agents with empty `allowedHeaders` keep the unchanged pure-M2M path (static SA token). This is the chart side of making the Slack-OBO sre-agent act as the human against muster; the downstream mcp-kubernetes/mcp-prometheus local-mint leg is unchanged. A `ci/test-agent-obo-values.yaml` render test covers the OBO agent (no `headersFrom`, no token-mint resources).
