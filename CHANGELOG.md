...
- **Agent creation (`agents.definitions`).** Agents are no longer created by this chart — install one release of the generic [`agent` chart](https://github.com/giantswarm/agent) per agent instead, per the [creating-agents PRD](https://github.com/giantswarm/bumblebee-plans/blob/main/creating-agents/PRD.md). The `agents.definitions` block is removed from the umbrella and connectivity `values.yaml`, along with the templates it drove in `agentic-platform-connectivity`: the kagent `Agent` CR + chart-owned `ServiceAccount` (`agents.yaml`), the per-agent `muster-<name>` `RemoteMCPServer` loop (the `agent` chart references the shared `muster` server cross-namespace instead), the per-agent A2A `AgentgatewayBackend` + `/agents/<name>` `HTTPRoute` (`agentgateway-backends.yaml` — not part of the PRD's chart scope and unused: in-cluster A2A callers go through the kagent controller route; use `extraObjects` for one-offs), and the now-unused `_agents.tpl` helpers. The admin-owned surface the `agent` chart depends on stays: the shared `muster` `RemoteMCPServer`, `agents.models` (ModelConfig catalog), `agents.remoteMcpServers`, and the kagent-namespace policies (muster-egress CNP, Kyverno exception, declarative-agent pod security), which apply to agent pods regardless of which chart created them. The `agents` schema deliberately stays `additionalProperties: true` so per-cluster values still carrying `agents.definitions` keep validating during migration. **Migration:** any cluster with `agents.definitions.sre-agent.enabled: true` must have an `agent`-chart-based replacement (carrying the eval-tuned `systemMessage`) deployed *before* upgrading, or the live Agent CR is deleted.
