...
- Kagent defaults hardened for GS clusters: restricted-PSS `securityContext` applied at umbrella level (Kyverno requirement); all bundled agents/tools disabled (redundant with GS stack — muster, mcp-observability-platform, mcp-prometheus cover the same surface); Anthropic set as the default model provider (`claude-sonnet-4-6`); OTel traces and logs routed to `otlp-gateway.kube-system.svc:4317`; `controller.auth.mode: unsecure` (agentgateway provides the JWT validation boundary); `oauth2-proxy` values pre-wired for Dex OIDC integration (`enabled: false` until Dex client credentials are provided).
