...
- `agents.definitions.<agent>`: an agent that forwards the inbound `Authorization` header to muster (listed in `allowedHeaders`) no longer also injects a static service-account token. Its muster `RemoteMCPServer` omits the `headersFrom` injection, and the per-agent token-mint Job, refresh CronJob, and Secret-writer RBAC are not rendered, so the caller's own token reaches muster. A request carries a single `Authorization` header, so the two are mutually exclusive; an agent that does not forward `authorization` keeps the previous static-token behaviour. A `ci/test-agent-forward-auth-values.yaml` render test covers this (no `headersFrom`, no token-mint resources).
